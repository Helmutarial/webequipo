...
const players: Player[] = [
  ...
  { id: "fj-garcia", name: "F.J. García", number: 14, position: "JUG", initials: "F" },
  { id: "juanorro", name: "Juanorro", number: 23, position: "JUG", initials: "J" },
  { id: "anglada", name: "Anglada", number: 80, position: "JUG", initials: "A" },
  { id: "molinpower", name: "MolinPower", number: 19, position: "JUG", initials: "M" },
  { id: "moreno", name: "Moreno", number: 22, position: "JUG", initials: "M" },
  { id: "luaces", name: "Luaces", number: 4, position: "JUG", initials: "L" },
...
export default function AlignmentBuilder() {
  ...
  const choosePlayer = (playerId: string) => {
    ...
    setLineup((current) => {
      ...
      if (pickerRole === "starter") return { ...current, [slotPicker]: [playerId, existing[1]].filter(Boolean) as string[] };
      if (existing.length >= 2) return current;
      return { ...current, [slotPicker]: [...existing, playerId] };
    });
    setSlotPicker(null); setSelectedPlayer(null); setSaved(false);
  };
